...
---@class VietnamChessPlayCoin
local VietnamChessPlayCoin=Class("VietnamChessPlayCoin")

---@type ObjectPoolUtil
local pool = ObjectPoolUtil:New("VietnamChessPlayCoin")
...
---
function VietnamChessPlayCoin:AnimateCoin(coin,coin_type,start_pos,target)
     -- 从对象池获取金币实例
    ---@type GameObject
...
---创建底注到区域
function VietnamChessPlayCoin:CreatCoinInArea(coin,coin_type,target)
    local coinObj = pool:Spawn(nil,coin)
...
function VietnamChessPlayCoin:DestroyCoinFly(coinObj,endPos)
    -- 创建动画序列
    local sequence = DOTween.Sequence()
...
function VietnamChessPlayCoin:Destroy()
    pool:DestroyAll()
end

return VietnamChessPlayCoin