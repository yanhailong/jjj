...
SlotMachineManager.__index = SlotMachineManager
...
function SlotMachineManager.New()
    local self = setmetatable({}, SlotMachineManager)
...
function SlotMachineManager:Create(root, elementIconPrefab)
    self.transform = root
...
function SlotMachineManager:Init(root)
    ...
    if not self.isOpenSingleAxisMask then
        ...
        CS.UnityEngine.Vector2(self.AxisOffset * self.AxisCount, self.ItemsPerAxis * self.ElementHeight + self.AxisSizeOffset)
    ...
    for i = 0, self.AxisCount - 1 do
        ...
        go.transform.localPosition = Vector3(-(self.AxisOffset * self.AxisCount)/2+self.AxisOffset/2,0,0) + CS.UnityEngine.Vector3(1, 0, 0) * self.AxisOffset * i
...
function SlotMachineManager:StartRollers()
...
function SlotMachineManager:StopAllRollers()
...
function SlotMachineManager:RollerCompleted(index)
...
function SlotMachineManager:SetDrop(WinPattern)
...
function SlotMachineManager:OnAllDropCompleted()
    self.State = GameState.DropCompleted
    self.DropCompleted_Event:Invoke()
end
function SlotMachineManager:SetCompletedState()
    self.State = GameState.Completed;
end

function SlotMachineManager:ChangeSpeed(rollerIndex, speed)
    self.axis[rollerIndex]:ChangeSpeed(speed)
end
function SlotMachineManager:Close()
