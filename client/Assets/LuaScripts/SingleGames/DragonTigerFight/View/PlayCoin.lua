...
function PlayCoin:DistroyCoin(targetPos,ratios)
    local results = distributeCoins(#coins,ratios)
    local index = 1
    local next = 0
    for i=1,#coins do
        if next + results[index] >= i then
            self:DistroyCoinFly(coins[i],targetPos[index])
        else
...
function PlayCoin:DistroyCoinFly(coinObj,endPos)
    -- 创建动画序列
    local sequence = DOTween.Sequence()

    -- 设置金币动画效果
    sequence:Append(coinObj.transform:DOMove(endPos, 0.8):SetEase(Ease.OutQuad))
    sequence:Join(coinObj.transform:DOScale(0.9, 0.8):SetEase(Ease.OutQuad))
    -- 动画完成后保持金币在桌面上
    sequence:OnComplete(function()
