...
function BirdsAnimalsGameView.CalcStepsToTarget(currentIndex, targetIndex, total)
    return (targetIndex - currentIndex + total) % total
end

function BirdsAnimalsGameView:StartMarquee(marqueeIndex,targetIndex)
...
function BirdsAnimalsGameView:MarqueeStep()
...
function BirdsAnimalsGameView:StopMarquee()
    self.marqueeRunning = false
    if self.marqueeTimer then
        TimerManager.StopTimer(self, self.marqueeTimer)
        self.marqueeTimer = nil
...
function BirdsAnimalsGameView:PlayRuningAnimtion(result, isPlay, callFunc)
    ...
    if isPlay then
        ...
        self:StartMarquee(result.last_logo.logo_index, result.win_logo.logo_index)
    else
...
function BirdsAnimalsGameView:PlayCarEffectView(logo_id,callFunc)
    ...
    self.img_icon.transform.localScale=Vector3(1,1,1)
    self.img_icon.transform:DOScale(Vector3.Zero(),0.3):SetEase(Ease.InBack):OnComplete(function()
