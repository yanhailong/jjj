...
---@class BirdsAnimalsPlayCoinView
local BirdsAnimalsPlayCoinView=Class("BirdsAnimalsPlayCoinView")

---@type ObjectPoolUtil
local pool = ObjectPoolUtil:New("BirdsAnimalsPlayCoinView")
...
---
function BirdsAnimalsPlayCoinView:AnimateCoin(coin,coin_type,start_pos,target)
     -- 从对象池获取金币实例
    ---@type GameObject
...
---创建底注到区域
function BirdsAnimalsPlayCoinView:CreatCoinInArea(coin,coin_type,target)
    local coinObj = pool:Spawn(nil,coin)
...
function BirdsAnimalsPlayCoinView:DestroyCoin(targetPos,ratios)
...
function BirdsAnimalsPlayCoinView:DestroyCoinFly(coinObj,endPos)
    -- 创建动画序列
    local sequence = DOTween.Sequence()
...
function BirdsAnimalsPlayCoinView:Destroy()
    pool:DestroyAll()
end

return BirdsAnimalsPlayCoinView