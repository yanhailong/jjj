...
--@desc 
--
---@class DicePointsSumSizeChipManager
local DicePointsSumSizeChipManager = Class("DicePointsSumSizeChipManager")
...
---@type ObjectPoolUtil
local pool = ObjectPoolUtil:New("DicePointsSumSizeChipManager")
...
---
function DicePointsSumSizeChipManager:AnimateChip(chip_type, start_pos, target)
    -- 从对象池获取金币实例
    ---@type #GameObject
    local chipObj = pool:SpawnPrefab(nil, DicePointsSumSizeConfig.ABNames.ChipPool, DicePointsSumSizeConfig.GetChipPoolName(chip_type), target.transform)
...
---创建底注到区域
function DicePointsSumSizeChipManager:CreatChipInArea(chip_type, target)
    local chipObj = pool:SpawnPrefab(nil, DicePointsSumSizeConfig.ABNames.ChipPool, DicePointsSumSizeConfig.GetChipPoolName(chip_type), target.transform)
...
function DicePointsSumSizeChipManager:DestroyChipFly(chipObj, endPos)
    if chipObj == nil then
        return
    end
    ...
    sequence:Play()
    DicePointsSumSizeChipManager:RemoveChip(chipObj)
end

function DicePointsSumSizeChipManager:CleanAllChipAnim()
...
function DicePointsSumSizeChipManager:CleanChip()
    for i = 1, #chips do
...
function DicePointsSumSizeChipManager:GetChipArr()
    return chips
end

function DicePointsSumSizeChipManager:RemoveChip(chipObj)
...
function DicePointsSumSizeChipManager:Destroy()
    --look("DicePointsSumSizeChipManager:Destroy()")
    DicePointsSumSizeChipManager:CleanAllChipAnim()
    DicePointsSumSizeChipManager:CleanChip()
    pool:DestroyAll()
end

return DicePointsSumSizeChipManager